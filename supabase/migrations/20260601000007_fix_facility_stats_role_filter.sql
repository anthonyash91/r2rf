-- Fix: total_session_seconds and items_completed_total in facility_stats were
-- only excluding facilityUser role accounts, not all staff. Every other subquery
-- in this function correctly excludes admin/contributor/tester/facilityUser.
-- In practice admins have NULL facility so they were already excluded by the
-- JOIN filter, but this corrects the inconsistency.
