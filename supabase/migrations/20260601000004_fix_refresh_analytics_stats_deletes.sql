-- Fix: replace bare DELETE FROM statements with DELETE FROM ... WHERE TRUE
-- so refresh_analytics_stats() can be called via the Supabase REST API (db.rpc).
-- pg_cron continues to call this function directly and was never affected.
...
BEGIN
  ...
  INSERT INTO facility_stats (facility_value, active_users_7d, active_users_30d, total_users, avg_completion_rate, total_session_seconds, items_completed_total, updated_at)
  SELECT f.value,
    ...
    (SELECT CASE WHEN GREATEST(fac_eng.cnt, fac_comp.cnt) > 0 THEN ROUND(fac_comp.cnt::numeric / GREATEST(fac_eng.cnt, fac_comp.cnt) * 100, 2) ELSE NULL END
     FROM (SELECT COUNT(*) AS cnt FROM (
       SELECT DISTINCT user_id, content_item_id FROM (
         SELECT ucs2.user_id, ucs2.content_item_id FROM user_content_sessions ucs2
         JOIN user_profiles up2 ON up2.user_id = ucs2.user_id AND up2.facility = f.value
         LEFT JOIN user_roles ur2 ON ur2.user_id = ucs2.user_id AND ur2.role IN ('admin','contributor','tester','facilityUser') WHERE ur2.user_id IS NULL
         UNION
         SELECT ae.user_id, ae.content_id FROM analytics_events ae
         JOIN user_profiles up2b ON up2b.user_id = ae.user_id AND up2b.facility = f.value
         LEFT JOIN user_roles ur2b ON ur2b.user_id = ae.user_id AND ur2b.role IN ('admin','contributor','tester','facilityUser')
         WHERE ur2b.user_id IS NULL AND ae.event_type = 'content_click' AND ae.user_id IS NOT NULL
       ) all_opens) eng_pairs) fac_eng,
    ...
    COALESCE((SELECT SUM(ucs2.session_seconds) FROM user_content_sessions ucs2
      JOIN user_profiles up2 ON up2.user_id = ucs2.user_id AND up2.facility = f.value
      LEFT JOIN user_roles ur2 ON ur2.user_id = up2.user_id AND ur2.role = 'facilityUser' WHERE ur2.user_id IS NULL), 0),
    COALESCE((SELECT COUNT(*) FROM user_content_progress ucp2
      JOIN user_profiles up3 ON up3.user_id = ucp2.user_id AND up3.facility = f.value
      LEFT JOIN user_roles ur3 ON ur3.user_id = up3.user_id AND ur3.role = 'facilityUser' WHERE ur3.user_id IS NULL), 0),
    now()
  FROM facilities f
  ...
  ON CONFLICT (facility_value) DO UPDATE SET
    ...
    total_session_seconds = EXCLUDED.total_session_seconds, items_completed_total = EXCLUDED.items_completed_total, updated_at = EXCLUDED.updated_at;
  ...
  LEFT JOIN user_content_progress ucp ON ucp.user_id = up.user_id
  ...
  -- Fixed: replaced bare DELETE FROM with DELETE FROM ... WHERE TRUE
  -- so this function can be called via db.rpc() through the PostgREST API.
  ...
  WITH eligible AS (
    ...
    SELECT ci.category_id, COUNT(*) AS total_items FROM content_items ci WHERE ci.published=true GROUP BY ci.category_id
    ...
    JOIN content_items ci ON ci.id=ucp.content_item_id AND ci.published=true
