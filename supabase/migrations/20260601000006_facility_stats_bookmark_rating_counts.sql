-- Add pre-computed bookmark and rating count columns to facility_stats so
-- getFacilityComparison can read them without fetching entire raw tables.
-- Previously, the TypeScript function fetched ALL rows from user_content_bookmarks
-- and user_content_ratings (silently truncated at 1000 rows by PostgREST) and
-- aggregated them in JavaScript. These columns are now computed by the nightly job.
ALTER TABLE public.facility_stats
  ...
  ADD COLUMN IF NOT EXISTS thumbs_down_count integer NOT NULL DEFAULT 0;

-- Rewrite refresh_analytics_stats() to populate the new facility_stats columns.
...
BEGIN
  ...
  SELECT f.value,
    ...
    -- Bookmark and rating counts per facility, excluding admin/facilityUser/tester accounts
    COALESCE((
