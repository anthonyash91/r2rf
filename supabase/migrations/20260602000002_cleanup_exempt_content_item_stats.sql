-- Remove stale content_item_stats rows for items that have been marked exempt.
-- When an item is marked exempt_from_progress = TRUE, the nightly
-- refresh_analytics_stats() skips inserting it, but any previously computed
-- row persists. This migration removes existing stale rows, and the updated
-- function below ensures future runs clean up on every execution.
DELETE FROM public.content_item_stats
WHERE content_item_id IN (
  SELECT id FROM public.content_items WHERE exempt_from_progress = TRUE
);

-- Update refresh_analytics_stats() to purge exempt items from content_item_stats
-- at the start of each run so items newly marked exempt are removed immediately.
...
BEGIN
  ...
  -- Remove stats for any items that are now marked exempt
  DELETE FROM content_item_stats
  WHERE content_item_id IN (
    SELECT id FROM content_items WHERE exempt_from_progress = TRUE
  );

  -- content_item_stats: only non-exempt items
