-- facility_stats.total_session_seconds only ever summed user_content_sessions
-- rows joined through user_profiles (i.e. registered users). Anonymous
-- sessions (user_id IS NULL, attributed only by facility_value — see the
-- anonymous-session-time migration from 2026-09-10) were never part of this
-- query at all, so they could never appear in the Facility Reports
-- comparison view, regardless of how correctly facility_value was set.
-- Only this one column's source query changes; every other section of the
-- function is reproduced unchanged because CREATE OR REPLACE FUNCTION
...
BEGIN
  ...
  -- facility_stats: total_session_seconds now adds anonymous sessions
  -- (user_id IS NULL) matched by facility_value alongside the existing
  -- registered-user sum — everything else in this section is unchanged.
  ...
  SELECT f.value,
    ...
    + COALESCE((SELECT SUM(ucs4.session_seconds) FROM user_content_sessions ucs4
      WHERE ucs4.user_id IS NULL AND ucs4.facility_value = f.value), 0),
...
-- Backfill: every anonymous row recorded before the facility-slug-casing fix
-- (setActiveFacilitySlug now normalizes to the same lowercase/slugified form
-- deriveValue() uses server-side) has facility_value set to whatever raw
-- casing the platform's siteID header happened to send — e.g. "S003007001"
-- instead of the canonical "s003007001" — so it never matched any real
-- facilities.value and was invisible to every facility-scoped report. Only
-- lowercase (not the fuller slugify) because every real siteID seen so far
-- is already alphanumeric; this only touches rows that don't already match
-- a real facility, so it's a no-op for anything already correct.
UPDATE public.user_content_sessions
  SET facility_value = LOWER(facility_value)
  WHERE facility_value IS NOT NULL
    AND facility_value NOT IN (SELECT value FROM public.facilities)
    AND LOWER(facility_value) IN (SELECT value FROM public.facilities);
...
-- analytics_daily_counts has a UNIQUE(event_type, category_id, content_id,
-- facility_value, period_date) constraint, so a plain UPDATE could collide
-- with an already-correctly-cased row for the same key. Merge (sum counts)
-- into the existing row if one exists, or create it, then drop the leftover
-- mis-cased original.
INSERT INTO public.analytics_daily_counts (event_type, category_id, content_id, facility_value, period_date, count)
SELECT event_type, category_id, content_id, LOWER(facility_value), period_date, count
FROM public.analytics_daily_counts bad
WHERE facility_value IS NOT NULL
  AND facility_value NOT IN (SELECT value FROM public.facilities)
  AND LOWER(facility_value) IN (SELECT value FROM public.facilities)
ON CONFLICT ON CONSTRAINT analytics_daily_counts_unique DO UPDATE SET
  count = analytics_daily_counts.count + EXCLUDED.count;

DELETE FROM public.analytics_daily_counts
  WHERE facility_value IS NOT NULL
    AND facility_value NOT IN (SELECT value FROM public.facilities)
    AND LOWER(facility_value) IN (SELECT value FROM public.facilities);

-- Apply immediately so the fix (and the backfill above) is visible without
-- waiting for the next scheduled run (see refresh_nightly_wrapper / pg_cron
-- for the schedule).
SELECT public.refresh_analytics_stats();
