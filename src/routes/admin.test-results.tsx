...
function RunDetailView({ runId }: { runId: string }) {
  ...
  const DEFAULT_ACTIVE =
    "relative z-10 text-[var(--color-accent)] bg-[var(--color-accent)]/10 border-[var(--color-accent)]/30";
  const DEFAULT_HOVER =
    "hover:text-[var(--color-accent)] hover:bg-[var(--color-accent)]/10 hover:border-[var(--color-accent)]/30";
            ...
            isFirst && isLast
              ...
              : isFirst
                ...
                : isLast
                  ...
                  : "rounded-none",
            active
              ? `relative z-10 ${activeClass}`
              : `bg-background text-muted-foreground border-border ${hoverClass}`,
          ]
            .filter(Boolean)
            .join(" ")}
        >
  ...
  return (
    <div>
      ...
      {failures.length > 0 && filterStatus !== "pass" && (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-5 mb-6">
          <p className="text-sm font-semibold text-red-700 mb-3">
            ...
            {failures.length !== 1 ? "require" : "requires"} attention
          </p>
      ...
      <div className="flex items-center gap-3 my-6 lg:hidden">
        <Select
          value={filterStatus}
          onValueChange={(v) => setFilterStatus(v as TestStatus | "all")}
        >
      ...
      <div className="hidden lg:flex items-center gap-3 my-6">
        <div className="flex items-center">
          {renderPill(
            (["all", "pass", "fail", "blocked", "skipped", "untested"] as const).map((s) => ({
              key: `s-${s}`,
        ...
        <div className="flex flex-col [&>div]:rounded-none [&>div:first-child]:rounded-t-2xl [&>div:last-child]:rounded-b-2xl [&>div:not(:first-child)]:-mt-px">
          {QA_SECTIONS.map((section) => {
            ...
            const filtered = sectionTests.filter((t) => {
              const statusMatch =
                filterStatus === "all" ||
                (resultMap.get(t.id)?.status ?? "untested") === filterStatus;
            ...
            const sActioned = sectionTests.filter(
              (t) => (resultMap.get(t.id)?.status ?? "untested") !== "untested",
            ).length;
            ...
            return (
              <div
                key={section.num}
                ref={(el) => {
                  if (el) sectionRefs.current.set(section.num, el);
                  else sectionRefs.current.delete(section.num);
                }}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-xs text-muted-foreground tabular-nums">
                      ...
                      {sPass > 0 && (
                        <span className="text-green-600 font-medium"> · {sPass} passed</span>
                      )}
                      {sFail > 0 && (
                        <span className="text-red-600 font-medium"> · {sFail} failed</span>
                      )}
                    </p>
                  </div>
                  <ChevronDown
                      ...
                      return (
                        <div
                          ...
                          style={{
                            backgroundColor:
                              status === "fail"
                                ? "color-mix(in oklab, #fee2e2 40%, transparent)"
                                : "#fffdf8",
                          }}
                        >
                          <StatusIcon
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                              ...
                              <div className="inline-flex items-center [&>span:not(:first-child)]:-ml-px [&>span:first-child]:rounded-r-none [&>span:not(:first-child):not(:last-child)]:rounded-none [&>span:last-child]:rounded-l-none [&>span:only-child]:rounded-[8px]">
                                <span
                                  className={`inline-flex items-center border px-2.5 py-[5px] text-xs font-medium flex-shrink-0 justify-center gap-1 rounded-[8px] ${pc.cls}`}
                                >
                                  <PIcon className="h-3.5 w-3.5" />
                                  {pc.label}
                                </span>
                                <span
                                  className={`inline-flex items-center border px-2.5 py-[5px] text-xs font-medium flex-shrink-0 justify-center gap-1 rounded-[8px] ${STATUS_COLORS[status]}`}
                                >
                                ...
                                {test.roles &&
                                  test.roles.map((role) => (
                                    <span
                                      key={role}
                                      className="inline-flex items-center gap-1 rounded-[8px] border border-border bg-background px-2.5 py-[5px] text-xs font-medium text-muted-foreground"
                                    >
                            ...
                            {(() => {
                              ...
                              return parts.length === 2 ? (
                                <>
                                  ...
                                  <p className="text-xs text-muted-foreground leading-relaxed whitespace-pre-line mt-1.5">
                                    ✅ Pass:{parts[1]}
                                  </p>
                                </>
                              ) : (
                            ...
                            {(res?.notes || (res as any)?.screenshot_url) && (
                              <div className="mt-[14px]">
                                {res?.notes && (
                                  <p className="text-xs text-muted-foreground rounded-md border border-input bg-background px-3 py-2 whitespace-pre-line">
                                    {res.notes}
                                  </p>
                                )}
      ...
      {selectedRunId ? (
        ...
        <>
          ...
          {!isLoading && runs.length === 0 && (
            <EmptyState>
              No test runs yet. Tester accounts will see a Testing tab on their dashboard.
            </EmptyState>
          )}
          {!isLoading && runs.length > 0 && (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span
                          className={`inline-flex items-center border px-2.5 py-[5px] text-xs font-medium flex-shrink-0 justify-center gap-1 rounded-[8px] ${
                            run.completed_at
                      ...
                      <div className="flex items-center gap-1.5">
                        {(counts.pass ?? 0) > 0 && (
                          <span
                            className="inline-flex items-center justify-center h-8 w-8 rounded-xl border text-xs font-semibold tabular-nums"
                            style={{
                              color: "oklch(0.52 0.12 165)",
                              backgroundColor:
                                "color-mix(in oklab, oklch(0.52 0.12 165) 12%, transparent)",
                              borderColor:
                                "color-mix(in oklab, oklch(0.52 0.12 165) 25%, transparent)",
                            }}
                          >
                            {counts.pass}
                          </span>
                        ...
                        {(counts.fail ?? 0) > 0 && (
                          <span
                            ...
                            style={{
                              ...
                              backgroundColor:
                                "color-mix(in oklab, oklch(0.55 0.15 25) 12%, transparent)",
                              borderColor:
                                "color-mix(in oklab, oklch(0.55 0.15 25) 25%, transparent)",
                            }}
                          >
                            {counts.fail}
                          </span>
                        ...
                        {(counts.blocked ?? 0) > 0 && (
                          <span
                            ...
                            style={{
                              ...
                              backgroundColor:
                                "color-mix(in oklab, oklch(0.60 0.12 80) 12%, transparent)",
                              borderColor:
                                "color-mix(in oklab, oklch(0.60 0.12 80) 25%, transparent)",
                            }}
                          >
                            {counts.blocked}
                          </span>
                        )}
                        {!(counts.pass ?? 0) && !(counts.fail ?? 0) && !(counts.blocked ?? 0) && (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-1.5">
                            {(counts.pass ?? 0) > 0 && (
                              <span
                                className="inline-flex items-center justify-center h-9 w-9 rounded-xl border text-xs font-semibold tabular-nums"
                                style={{
                                  color: "oklch(0.52 0.12 165)",
                                  backgroundColor:
                                    "color-mix(in oklab, oklch(0.52 0.12 165) 12%, transparent)",
