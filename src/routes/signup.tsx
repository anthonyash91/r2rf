...
// post-auth redirect. Delegates all form rendering to the two sub-components.
...
// Sign-in and sign-up share the same form element and submit handler —
// splitting them further would duplicate username/password state and logic.
...
function SignInSignUpForm({
  ...
  const submitSignupFn = useServerFn(signupUser);
    ...
    try {
      if (mode === "sign-up") {
        ...
        if (!Number.isFinite(ans)) {
          setAnswerError(t("signup.answerVerification"));
          return;
        }
        ...
        if (authedUser) {
          ...
          if (roleRow) {
            ...
            // Regular user via facility link — verify they belong to this facility.
      ...
      <div className="rounded-lg border border-border bg-[#fffdf8] px-6 pt-4 pb-6">
        {resetStep === 1 ? (
          ...
          <form onSubmit={handleResetSubmit} className="space-y-4">
            <div>
              <label htmlFor="reset-answer-1" className="text-sm font-medium">
            ...
            <div>
              <label htmlFor="reset-answer-2" className="text-sm font-medium">
