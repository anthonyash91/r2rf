                  ...
                  {data.items.map((item) => {
                    ...
                    return (
                      <li key={item.id} id={`item-${item.id}`} className="relative scroll-mt-24">
                        {isNew && (
                          <span className={`absolute top-3 z-10 inline-flex items-center gap-1 rounded-full bg-[var(--color-accent)] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-background shadow-sm ${isAdmin ? "right-[6.5rem]" : "right-3"}`}>
