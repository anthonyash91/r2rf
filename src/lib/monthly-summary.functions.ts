...
export const getMyMonthlySummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [thisProgressRes, lastProgressRes, thisTimeRes, lastTimeRes, thisAchievementsRes] =
      await Promise.all([
        ...
        db.from("user_achievements").select("achievement_key").gte("earned_at", firstOfThisMonth),
      ]);

    const itemsThisMonth = thisProgressRes.count ?? 0;
    const itemsLastMonth = lastProgressRes.count ?? 0;

    const secondsThisMonth = ((thisTimeRes.data ?? []) as any[]).reduce(
      (s: number, r: any) => s + (r.session_seconds || 0),
      0,
    );
    const secondsLastMonth = ((lastTimeRes.data ?? []) as any[]).reduce(
      (s: number, r: any) => s + (r.session_seconds || 0),
      0,
    );

    const achievementKeysThisMonth: string[] = ((thisAchievementsRes.data ?? []) as any[]).map(
      (r: any) => r.achievement_key as string,
    );
...
export const getAdminUserMonthlySummary = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const secondsThisMonth = ((thisTime.data ?? []) as any[]).reduce(
      (s: number, r: any) => s + (r.session_seconds || 0),
      0,
    );
    const secondsLastMonth = ((lastTime.data ?? []) as any[]).reduce(
      (s: number, r: any) => s + (r.session_seconds || 0),
      0,
    );
    const achievementKeysThisMonth: string[] = ((thisAchievements.data ?? []) as any[]).map(
      (r: any) => r.achievement_key as string,
    );
    const achievementsThisMonth = achievementKeysThisMonth.length;
