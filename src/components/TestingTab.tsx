...
function TestingTab() {
  ...
  const resultMap = new Map<
    string,
    { status: TestStatus; notes: string | null; screenshot_url: string | null }
  >();
  for (const r of resultsQuery.data?.results ?? []) {
    resultMap.set(r.test_id, {
  ...
  async function handleDeleteRun(runId: string, label: string) {
    await confirmDelete({
      title: `Delete "${label}"?`,
      description:
        "This will permanently delete the test run and all its results, statuses, notes, and screenshots.",
      onConfirm: async () => {
      ...
      <div>
        ...
        {creating && (
          <div className="mb-6 rounded-2xl border border-border bg-card p-5 flex items-center gap-3">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") handleCreateRun();
                if (e.key === "Escape") {
                  setCreating(false);
                  setNewLabel("");
                }
        ...
        {runsQuery.isLoading ? (
          ...
          <div className="rounded-2xl border border-dashed border-border p-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Create a new run to start working through the QA checklist.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {runs.map((run: any) => {
              ...
              return (
                ...
                >
                  <ClipboardCheck
                    className={`h-5 w-5 shrink-0 ${run.completed_at ? "text-green-600" : "text-[var(--color-accent)]"}`}
                  />
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      handleDeleteRun(run.id, run.label);
                    }}
              ...
              {isOpen && (
                <div className="border-t border-border divide-y divide-border/60">
                  {filtered.map((test) => {
                    ...
                    const noteDirty =
                      pendingNotes[test.id] !== undefined &&
                      pendingNotes[test.id] !== (result?.notes ?? "");
                            ...
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                              ...
                              <div className="inline-flex items-center [&>span:not(:first-child)]:-ml-px [&>span:first-child]:rounded-r-none [&>span:not(:first-child):not(:last-child)]:rounded-none [&>span:last-child]:rounded-l-none [&>span:only-child]:rounded-[8px]">
                                {(() => {
                                  ...
                                  return (
                                    <span
                                      className={`inline-flex items-center border px-2.5 py-[5px] text-xs font-medium flex-shrink-0 justify-center gap-1 rounded-[8px] ${priorityConfig.cls}`}
                                    >
                                ...
                                <div className="flex items-center mt-4 ml-7">
                                  ...
                                  {(
                                    ...
                                    return (
                                      <span
                                        key={s}
                                        onClick={() => {
                                          if (!saving) handleSetStatus(test.id, s);
                                        }}
                                        className={[
                                          "inline-flex items-center border px-2.5 py-[5px] text-xs font-medium flex-shrink-0 justify-center gap-1 cursor-pointer transition-colors",
                                          !isFirst && "-ml-px",
                                  ...
                                  {status !== "fail" && status !== "blocked" && (
                                    <span
                                      onClick={() =>
                                        notesOpen
                                          ? handleRemoveNote(test.id)
                                          : setOpenNotes((prev) => new Set(prev).add(test.id))
                                      }
                                ...
                                {notesOpen && (
                                  <div className="ml-7 mt-[14px]">
                                    <textarea
                                      ...
                                      onBlur={() => {
                                        if (noteDirty) handleSaveNote(test.id);
                                      }}
                                    ...
                                    <div className="flex items-center justify-between gap-2 mt-[8px]">
                                      ...
                                      <span
                                        onClick={() => {
                                          if (currentNote.trim()) handleSaveNote(test.id);
                                        }}
