...
import {
  ...
  resetFacilityAnalytics,
} from "@/lib/analytics-stats.functions";
...
export function UsageReportView({ scope }: { scope: UsageScope }) {
  ...
  const aggregated = useMemo(() => {
    ...
    const rows: AggregatedRow[] = (d.categories ?? []).map((cat: Category) => {
      ...
      return {
        category: cat,
        views: catViews[cat.id] ?? 0,
        clicks: catClicks[cat.id] ?? 0,
        completionRate: catCompletionRate[cat.id] ?? null,
        totalSeconds: catTotalSeconds[cat.id] ?? 0,
        depth: catDepth[cat.id] ?? null,
        items,
      };
    });
    return {
      ...
      overallCompletionRate: d.overallCompletionRate ?? null,
      typeStats,
    };
  }, [data]);
                ...
                <LoadingButton
                  ...
                  onClick={async () => {
                    await confirm({
                      ...
                      description:
                        "This permanently deletes all completions, engagement time, logins, achievements, and pre-computed stats for every user in the CPC Sales facility. Run this before a QA test session to start from a clean baseline. This cannot be undone.",
                      ...
                      onConfirm: async () => {
                        setIsResetting(true);
                        try {
                          const result = await doResetFacility({
                            data: { facilityValue: "s003007001" },
                          });
                          await qc.invalidateQueries();
                          toast.success(
                            `CPC Sales analytics reset — cleared data for ${(result as any).clearedUsers} users.`,
                          );
                        } catch (err: any) {
        ...
        <>
          <div className="mt-8 grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-5">
            <SummaryCard
              ...
              value={
                aggregated.overallCompletionRate != null
                  ? `${aggregated.overallCompletionRate}%`
                  : "—"
              }
              tooltip="Out of everyone who opened content in this period, how many actually finished it."
            />
            {(() => {
              const timeVal = formatTimeSpent(
                (data as any)?.totalSeconds ?? ((data as any)?.hoursSpent ?? 0) * 3600,
              );
            ...
            <SummaryCard
              ...
              label={
                scope.kind === "facility"
                  ? (data as any)?.facilityUserCount === 1
                    ? "User"
                    : "Users"
                  : (data as any)?.totalUsers === 1
                    ? "User"
                    : "Users"
              }
              value={
          ...
          <RetentionSection
            retention={growthData?.retention ?? null}
...
function exportUsageCsv(
  ...
  lines.push(
    [
      "Completion rate",
      aggregated.overallCompletionRate != null ? `${aggregated.overallCompletionRate}%` : "",
    ]
      .map(csvEscape)
      .join(","),
  );
  lines.push(
    ["Time spent", formatTimeSpent(summary.totalSeconds ?? summary.hoursSpent * 3600)]
      .map(csvEscape)
      .join(","),
  );
  ...
  if (typeRows.length > 0) {
    ...
    lines.push(
      ["Type", "Items", "Opens", "Completions", "Completion rate", "Time spent"]
        .map(csvEscape)
        .join(","),
    );
...
function SummaryCard({
  ...
  return (
    <SectionCard as="div" padded={false} className="p-5">
      ...
      <p
        className={`mt-2 font-display font-semibold tabular-nums whitespace-nowrap ${valueClassName ?? "text-3xl"}`}
      >
...
function Stat({
  ...
  const inner = (
    <span
      className={`inline-flex items-center gap-1 rounded-[8px] border border-border bg-background px-2.5 py-[5px] text-xs font-medium${tooltip ? " cursor-default" : ""}`}
    >
      {icon}
...
function StatGridCell({
  ...
  const inner = (
    <div
      className={`flex items-center gap-2.5 rounded-lg border border-border bg-background px-3 py-2.5 ${className ?? ""}`}
    >
        ...
        >
          ...
          <div className="@[800px]:hidden grid grid-cols-2 gap-2 w-full mt-2">
            <StatGridCell
              ...
              label={row.views === 1 ? "visit" : "visits"}
              value={row.views}
              tooltip="How many times this category page was viewed in the selected period."
            />
            <StatGridCell
              icon={<MousePointerClick className="h-3.5 w-3.5" />}
              label={row.clicks === 1 ? "open" : "opens"}
              value={row.clicks}
              tooltip="How many times content items in this category were opened in the selected period."
            />
            {(() => {
                ...
                <>
                  {row.completionRate != null && (
                    <StatGridCell
                      ...
                      suffix="%"
                      tooltip="Of everyone who opened content in this category, how many finished it."
                      className={!hasBoth ? "col-span-2" : ""}
                    />
                  )}
                  {row.depth != null && (
                    <StatGridCell
                      ...
                      label="depth"
                      value={row.depth}
                      tooltip="Average number of items completed per user who engaged with this category in the selected period."
                      className={!hasBoth ? "col-span-2" : ""}
                    />
                  )}
                </>
            ...
            <StatGridCell
              ...
              label="time spent"
              value={row.totalSeconds > 0 ? formatTimeSpent(row.totalSeconds) : null}
              tooltip="Total time all users spent on content in this category in the selected period."
                  ...
                  <div className="flex flex-col @[700px]:flex-row @[700px]:items-center gap-3 px-6 py-[19px]">
                          ...
                          {item.exempt_from_progress && (
                            <Tooltip>
                              ...
                              <TooltipContent className="max-w-xs px-3 py-2">
                                Exempt from tracking — this item doesn't count toward user progress
                                or completion rates.
                              </TooltipContent>
                            </Tooltip>
                          )}
                    ...
                    <div className="hidden @[700px]:flex items-center flex-shrink-0 flex-nowrap [&>span:not(:first-child)]:-ml-px [&>span:first-child]:rounded-r-none [&>span:not(:first-child):not(:last-child)]:rounded-none [&>span:last-child]:rounded-l-none [&>span:only-child]:rounded-[8px]">
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs px-3 py-2">
                          Opens — how many times this item was clicked in the selected period.
                        </TooltipContent>
                      </Tooltip>
                      {completionRate != null && (
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-xs px-3 py-2">
                            Of everyone who opened this item, how many finished it.
                          </TooltipContent>
                        </Tooltip>
                      )}
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span className="inline-flex items-center gap-1 border border-border bg-background px-2.5 py-[5px] text-xs font-medium rounded-[8px] tabular-nums cursor-default">
                              ...
                              {openCount - completeCount} drop-off
                              {openCount - completeCount === 1 ? "" : "s"}
                            </span>
                          </TooltipTrigger>
                          <TooltipContent className="max-w-xs px-3 py-2">
                            Drop-offs — users who opened this item but did not complete it.
                          </TooltipContent>
                        </Tooltip>
                      )}
                      ...
                      {(thumbsUp > 0 || thumbsDown > 0) && (
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-xs px-3 py-2">
                            Helpful / Not helpful ratings from users.
                          </TooltipContent>
                        </Tooltip>
                      )}
                      {bookmarkCount > 0 && (
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-xs px-3 py-2">
                            Users who have bookmarked this item.
                          </TooltipContent>
                        </Tooltip>
                      )}
                    </div>
                    {(() => {
                      ...
                      const cells = [
                        <StatGridCell
                          ...
                          label="opens"
                          value={clicks}
                          tooltip="How many times this item was clicked in the selected period."
                        />,
                        completionRate != null && (
                          <StatGridCell
                            key="comp"
                            ...
                            suffix="%"
                            tooltip="Of everyone who opened this item, how many finished it."
                          />
                        ),
                        openCount > completeCount && (
                          <StatGridCell
                        ...
                        hasRatings && (
                          <StatGridCell
                            ...
                            tooltip="Helpful / Not helpful ratings from users."
                          />
                        ),
                        bookmarkCount > 0 && (
                          <StatGridCell
                            ...
                            label="bookmarks"
                            value={bookmarkCount}
                            tooltip="Users who have bookmarked this item."
                          />
                        ),
...
function MostLeastEngaged({ rows }: { rows: AggregatedRow[] }) {
  const allItems = rows.flatMap((r) =>
    r.items.filter((i) => i.openCount >= 3).map((i) => ({ ...i, categoryName: r.category.name })),
  );
        ...
        {items.map(({ item, completionRate, openCount, categoryName }) => (
          <li key={item.id} className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex flex-col items-end flex-shrink-0 gap-0.5">
              <span
                className={`text-sm font-semibold tabular-nums ${accent ? "text-[var(--color-accent)]" : "text-muted-foreground"}`}
              >
                {completionRate}%
              </span>
      ...
      <Tabs defaultValue="most">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          ...
          <TabsList className="h-auto p-2 gap-1 self-start sm:self-auto bg-muted/40 border border-border rounded-lg">
            <TabsTrigger
              value="most"
              className="px-4 py-2 text-sm data-[state=active]:shadow-none hover:bg-background hover:text-foreground"
            >
              Most engaged
            </TabsTrigger>
            {least.length > 0 && (
            ...
            <thead>
              <tr className="border-b border-border bg-muted/30 text-xs text-muted-foreground font-medium">
                ...
                <th className="text-left px-4 py-3">
                  <span className="inline-flex items-center gap-1">
                    Items <InfoTooltip text="Number of published content items of this type." />
                  </span>
                </th>
                <th className="text-left px-4 py-3">
                  <span className="inline-flex items-center gap-1">
                    Opens{" "}
                    <InfoTooltip text="How many times items of this type were opened in the selected period." />
                  </span>
                </th>
                <th className="text-left px-4 py-3">
                  <span className="inline-flex items-center gap-1">
                    Completions{" "}
                    <InfoTooltip text="How many times items of this type were completed in the selected period." />
                  </span>
                </th>
                <th className="text-left px-4 py-3">
                  <span className="inline-flex items-center gap-1">
                    Completion Rate{" "}
                    <InfoTooltip text="Percentage of opens that resulted in completion." />
                  </span>
                </th>
                <th className="text-left px-4 py-3">
                  <span className="inline-flex items-center gap-1">
                    Time Spent{" "}
                    <InfoTooltip text="Total accumulated session time across all users on this content type. All time." />
                  </span>
                </th>
              </tr>
            </thead>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30 text-xs text-muted-foreground font-medium">
                ...
                <th className="text-left px-4 py-3">
                  <span className="inline-flex items-center gap-1">
                    Items <InfoTooltip text="Total published content items in this program." />
                  </span>
                </th>
                <th className="text-left px-4 py-3">
                  <span className="inline-flex items-center gap-1">
                    Started{" "}
                    <InfoTooltip text="Users who have completed at least one item in this program." />
                  </span>
                </th>
                <th className="text-left px-4 py-3">
                  <span className="inline-flex items-center gap-1">
                    Completed{" "}
                    <InfoTooltip text="Users who have completed every single item in this program." />
                  </span>
                </th>
                <th className="text-left px-4 py-3">
                  <span className="inline-flex items-center gap-1">
                    Completion Rate{" "}
                    <InfoTooltip text="Percentage of started users who finished the entire program. A user must complete all items to count." />
                  </span>
                </th>
              </tr>
            </thead>
    ...
    <div className="mt-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <SummaryCard
          ...
          value={day7 != null ? `${day7}%` : "—"}
          tooltip="Of users who signed up more than 7 days ago, the percentage who logged back in at least once within their first 7 days."
        />
        <SummaryCard
          icon={<UsersIcon className="h-5 w-5" />}
          label="30-day return rate"
          value={day30 != null ? `${day30}%` : "—"}
          tooltip="Of users who signed up more than 30 days ago, the percentage who logged back in at least once within their first 30 days."
        />
        <SummaryCard
          icon={<UsersIcon className="h-5 w-5" />}
          label="60-day return rate"
          value={day60 != null ? `${day60}%` : "—"}
          tooltip="Of users who signed up more than 60 days ago, the percentage who logged back in at least once within their first 60 days."
        />
      </div>
    </div>
  );
            ...
            <thead>
              <tr className="border-b border-border bg-muted/30 text-xs text-muted-foreground font-medium">
                ...
                <th className="text-left px-4 py-3 w-64">
                  <span className="inline-flex items-center gap-1">
                    New signups{" "}
                    <InfoTooltip text="Users who created an account during this week." />
                  </span>
                </th>
                <th className="text-left px-4 py-3 w-64">
                  <span className="inline-flex items-center gap-1">
                    Active users{" "}
                    <InfoTooltip text="Unique users who opened at least one piece of content during this week." />
                  </span>
                </th>
              </tr>
            </thead>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(w.activeUsers / maxActive) * 100}%` }}
                        />
                      </div>
